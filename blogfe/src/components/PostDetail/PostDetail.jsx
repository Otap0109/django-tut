...
async function deletePost(id) {
    const response = await axios.delete(`http://127.0.0.1:8000/api/v1/article/${id}/`);
...
export default function PostDetail() {
    ...
    return (
        <div className={styles.container}>
            ...
            {mutation.isError && <div className={styles.error}>Error: {mutation.error.message}</div>}
            {mutation.isSuccess && <div className={styles.success}>Post deleted successfully!</div>}
        </div>
    );
}
